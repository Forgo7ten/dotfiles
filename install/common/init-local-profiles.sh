...
# 初始化机器本地的 shell profile 文件（.bash_aliases / .pre_profile / .post_profile）
...
function init_local_profiles() {
    ...
    if [ ! -f "$HOME/.pre_profile" ]; then
        touch "$HOME/.pre_profile"
    fi

    # .post_profile 不存在时，创建并写入 .bash_aliases 加载片段
    if [ ! -f "$HOME/.post_profile" ]; then
        cat > "$HOME/.post_profile" <<'EOF'
